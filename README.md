...
1. **Semantic Q&A matching** — queries are matched against a curated knowledge base of question–answer pairs using sentence embeddings and FAISS vector search.
2. **Passage search fallback** — if no Q&A match is found, relevant passages are retrieved from uploaded PDFs using a separate chunk index.
...
1. **Upload PDFs** — go to Documents → PDFs and upload university PDF documents.
2. **Generate Q&A suggestions** — the system automatically generates question–answer pairs from the uploaded text.
3. **Review suggestions** — approve or reject generated pairs in the review panel.
4. **Update knowledge base** — approved pairs are embedded and indexed, making them searchable.
...
- Query characteristics by length
- Gap analysis (unanswered queries)
- Score comparison across retrieval modes
